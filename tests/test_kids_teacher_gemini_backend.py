...
# ---------------------------------------------------------------------------
# Readiness signal — Gemini Live's WebSocket handshake completes before the
# server has finished setting up the session. Anything pushed pre-readiness
# (audio, text) is silently dropped on the server side. The first
# ``session_resumption_update`` is the practical "session hot" signal.
# ---------------------------------------------------------------------------


def _build_session_resumption_message(
    *, new_handle: str = "handle-001", resumable: bool = True
) -> Any:
    """Build a top-level message containing only a session_resumption_update.

    Mirrors the shape Gemini Live emits — no server_content, just the SR
    update — so :meth:`GeminiRealtimeBackend._normalize_message` exercises
    the dedicated branch that flips the readiness event.
    """
...
def test_ready_event_clear_until_first_session_resumption_update() -> None:
    backend = _new_backend()
    assert backend._ready_event.is_set() is False

    backend._normalize_message(_build_session_resumption_message())

    assert backend._ready_event.is_set() is True


def test_ready_event_remains_set_across_subsequent_updates() -> None:
    """Once flipped the event must stay set — backend treats the first
    update as a one-way transition. Subsequent SR updates rotate the
    resumption handle but the readiness contract doesn't oscillate."""
    backend = _new_backend()
    backend._normalize_message(_build_session_resumption_message(new_handle="h1"))
    backend._normalize_message(_build_session_resumption_message(new_handle="h2"))

    assert backend._ready_event.is_set() is True


def test_ready_event_not_set_by_input_transcription() -> None:
    """Pre-readiness server messages other than session_resumption_update
    must not flip the gate — otherwise we'd un-gate the warm-up window."""
    backend = _new_backend()
    backend._normalize_message(
        _build_server_message(input_text="hello", input_finished=False)
    )
...
@pytest.mark.asyncio
async def test_wait_until_ready_unblocks_on_first_session_resumption_update() -> None:
    """End-to-end: a coroutine awaiting wait_until_ready unblocks exactly
    when the first SR update lands, not on connect()."""
    backend = _new_backend()
    ...
    # Give the waiter a chance to enter wait_until_ready.
