...
# ---------------------------------------------------------------------------
# Mic pump readiness gating
#
# Until the backend signals "session warm", the pump must drain the mic
# source (so the buffer doesn't overflow with stale audio) but NOT forward
# anything to the handler. Without this gate, the first ~30s of each Gemini
# Live boot drops audio into a void — exactly the warm-up dead zone visible
# in myra.log on 2026-04-29.
# ---------------------------------------------------------------------------


class _GatedHandler:
    """FakeHandler with a controllable readiness event."""
...
async def test_mic_pump_drops_frames_until_handler_signals_ready():
    """The pump must not forward audio to the handler before readiness —
    pre-warmup frames should be drained from the mic and discarded."""
    handler = _GatedHandler(ready=False)
    stop_event = asyncio.Event()

    pre_ready_chunks = [b"\x01" * 4, b"\x02" * 4, b"\x03" * 4]
    post_ready_chunks = [b"\xaa" * 4, b"\xbb" * 4]
    schedule = list(pre_ready_chunks)
    flipped = {"done": False}

    def reader() -> Optional[bytes]:
        if not flipped["done"]:
            if schedule:
                return schedule.pop(0)
            # Pre-ready buffer drained — flip readiness and seed post-ready.
            flipped["done"] = True
            handler.mark_ready()
            schedule.extend(post_ready_chunks)
            return None
        if schedule:
            return schedule.pop(0)
        stop_event.set()
    ...
    # Only post-ready frames reached the handler.
    assert handler.pushed == post_ready_chunks


async def test_mic_pump_skips_readiness_gate_when_disabled():
    """Existing tests + headless callers can opt out via wait_for_ready=False
    so this gate doesn't ripple through every backend that lacks the new
    readiness contract."""
    handler = _GatedHandler(ready=False)  # never flips ready
    ...
    # Without the gate, the single frame reaches the handler immediately.
    await asyncio.wait_for(
        pump_microphone_to_backend(
            handler, mic_source=reader, wait_for_ready=False
        ),
        timeout=1.0,
    )
...
async def test_mic_pump_exits_during_warmup_when_stop_event_fires():
    """Shutdown during the warm-up wait must end the pump cleanly without
    flushing pre-warmup audio."""
    handler = _GatedHandler(ready=False)
    stop_event = asyncio.Event()
    ...
    # Nothing forwarded — readiness never arrived.
    assert handler.pushed == []
...
# ---------------------------------------------------------------------------
# Startup greeting helper (mirrors the recovery cue tests above).
#
# The greeting plays a hardcoded "robot teacher awake" line through the local
# TTS path so the child has an audible "ready to listen" signal before the
# Gemini Live session is fully warmed up.
# ---------------------------------------------------------------------------


def test_default_startup_greeting_calls_dino_voice_and_plays(monkeypatch):
    """The default greeting fetches the fixed phrase via api_get_dino_voice
    and plays the resulting MP3 bytes through the robot speaker."""
    import kids_teacher_flow as flow
    ...
    robot = FakeRobotController()
    flow._default_startup_greeting(robot)

    assert fetched_text == [flow._STARTUP_GREETING_TEXT]
    assert played == [(robot, b"FAKEMP3")]


def test_default_startup_greeting_swallows_tts_failure(monkeypatch):
    """A TTS outage on boot must not crash the session — the greeting is
    best-effort, not a precondition for the realtime loop."""
    import kids_teacher_flow as flow
    ...
    flow._default_startup_greeting(FakeRobotController())  # must not raise


def test_default_startup_greeting_swallows_missing_robot_teacher(monkeypatch):
    """When ``robot_teacher`` (and its audio deps) is absent the greeting
    must silently no-op rather than crash the flow at session start."""
    import kids_teacher_flow as flow
...
def test_resolve_startup_greeting_delay_uses_env_override(monkeypatch):
    """The delay is configurable via env so deployments with slower Gemini
    warm-up can stretch the wait without a code change."""
    import kids_teacher_flow as flow

    monkeypatch.setenv(flow._STARTUP_GREETING_DELAY_ENV, "1.5")
    assert flow._resolve_startup_greeting_delay() == 1.5


def test_resolve_startup_greeting_delay_falls_back_on_garbage(monkeypatch):
    """A non-float env value must fall back to the default rather than
    crashing at flow import time."""
    import kids_teacher_flow as flow

    monkeypatch.setenv(flow._STARTUP_GREETING_DELAY_ENV, "not-a-number")
    assert (
        flow._resolve_startup_greeting_delay()
        == flow._DEFAULT_STARTUP_GREETING_DELAY
    )


def test_resolve_startup_greeting_delay_clamps_negative(monkeypatch):
    """Negative delays would put ``asyncio.sleep`` into immediate-return mode
    on most loops but the contract is "wait then play" — clamp to 0."""
    import kids_teacher_flow as flow

    monkeypatch.setenv(flow._STARTUP_GREETING_DELAY_ENV, "-2")
    assert flow._resolve_startup_greeting_delay() == 0.0
